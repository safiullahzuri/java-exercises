...
public class LinkedListPane2 extends Pane {

    private LinkedList<Integer> list;
    private Text tMessage;

    public LinkedListPane2(){
        setUp();
    }
    ...
    private void drawPane(){
        ...
        final double LINK_WIDTH = 20;

        for (int i=0; i<list.size(); i++){
            Rectangle slotRect =
                    new Rectangle(
                            xPos, yPos, SLOT_WIDTH, SLOT_HEIGHT
                    );
            Text slotText = new Text(
                    slotRect.getX()+10, slotRect.getY()+10, list.get(i)+""
            );

            xPos += SLOT_WIDTH;

            Rectangle linkRect = new Rectangle(xPos, yPos, SLOT_WIDTH/2, SLOT_HEIGHT);
            ...
            if (i < list.size() - 1){
                ...
                connection.setStartY(linkRect.getX()+(SLOT_HEIGHT/2));
    ...
    public void delete(int value) {
        if(list.remove(new Integer(value))){
            ...
            tMessage.setText(value +" not found in the list.");
        }
    }
}
