...
public class ExternalSortDemo {
    ...
    public static void main(String[] args) throws Exception{
        new File("largedata.dat").createNewFile();
    ...
    public static int initializeSegments(int segmentSize, String originalFile, String f1) throws Exception{
        ...
        DataInputStream dataInputStream = new DataInputStream(new BufferedInputStream(new FileInputStream(originalFile)));
        DataOutputStream dataOutputStream = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(f1)));
        ...
        while (dataInputStream.available() > 0){
            ...
            for (; dataInputStream.available()>0; i++){
                list[i] = dataInputStream.readInt();
            }
            //sort an array
            Arrays.sort(list, 0, i);
            for (int j=0; j<i; j++){
                dataOutputStream.writeInt(list[j]);
            }
        }
        dataInputStream.close();
